...
async function getOrCreateSumReward(accountId: string): Promise<SumReward> {
  let entity = await SumReward.get(accountId);
  if (entity === undefined) {
    entity = new SumReward(accountId);
...
export async function handleSumRewarded(event: SubstrateEvent): Promise<void> {
  ...
  let entity = await getOrCreateSumReward(account.toString());
